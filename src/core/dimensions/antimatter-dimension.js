...
export function antimatterDimensionCommonMultiplier() {
  ...
  multiplier = multiplier.times(getAdjustedGlyphEffect("powermult").times(TMTE(68)));
  multiplier = multiplier.times(Currency.realityMachines.value.powEffectOf(AlchemyResource.force).times(TMTE(69)));
...
export function getDimensionFinalMultiplierUncached(tier) {
  ...
  if (Effarig.isRunning) {
    ...
    multiplier = multiplier.pow(TMTE(70));
  }
  ...
  if (AlchemyResource.inflation.isUnlocked && multiplier.gte(AlchemyResource.inflation.effectValue)) {
    multiplier = multiplier.pow(TMTE(71));
  }
...
function applyNDPowers(mult, tier) {
  ...
  multiplier = multiplier.pow(glyphPowMultiplier * glyphEffarigPowMultiplier * Ra.momentumValue * TMTE(73));
