...
export class Galaxy {
  static get remoteStart() {
    return Math.floor(TMTE(117) * RealityUpgrade(21).effectOrDefault(800));
  }
  ...
  static requirementAt(galaxies) {
    ...
    if (type === GALAXY_TYPE.REMOTE) {
      amount *= Math.pow((1.002 * TMTE(117)), galaxies - (Galaxy.remoteStart - 1));
    }
  ...
  static get baseCost() {
    return (NormalChallenge(10).isRunning ? 19 : 0) + TMTE(118);
  }
...
function galaxyReset() {
  ...
  if(Math.random() * 100 >= TMTE(119)) player.galaxies++;
...
function maxBuyGalaxies(limit = Number.MAX_VALUE) {
  ...
  if(Math.random() * 100 >= TMTE(119)) galaxyReset();
