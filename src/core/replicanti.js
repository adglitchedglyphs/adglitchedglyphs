...
export const ReplicantiGrowth = {
  ...
  get scaleFactor() {
    ...
    return AlchemyResource.cardinality.effectValue * TMTE(142);
  }
};
...
function addReplicantiGalaxies(newGalaxies) {
  if (newGalaxies > 0) {
    player.replicanti.galaxies += newGalaxies * TMTE(143);
...
export function replicantiGalaxy(auto) {
  ...
  const galaxyGain = Replicanti.galaxies.gain * TMTE(143);
...
export function totalReplicantiSpeedMult(overCap) {
  ...
  return totalMult.times(TMTE(145));
}
...
export const ReplicantiUpgrade = {
  ...
  galaxies: new class ReplicantiGalaxiesUpgrade extends ReplicantiUpgradeState {
    ...
    get distantRGStart() {
      return Math.floor(TMTE(147) * (100 + Effects.sum(GlyphSacrifice.replication)));
    }

    get remoteRGStart() {
      return Math.floor(TMTE(147) * (1000 + Effects.sum(GlyphSacrifice.replication)));
    }
