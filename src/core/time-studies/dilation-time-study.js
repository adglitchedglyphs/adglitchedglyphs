...
export class DilationTimeStudyState extends TimeStudyState {
  ...
  purchase(quiet = false) {
    ...
    if (this.id === 1) {
      ...
      if (Perk.autounlockDilation1.canBeApplied && !TMTE(103)) {
        for (const id of [4, 5, 6]) player.dilation.upgrades.add(id);
      }
      if (Perk.autounlockDilation2.canBeApplied && !TMTE(103)) {
    ...
    if (this.id === 6) {
      ...
      if (!PlayerProgress.realityUnlocked() || TMTE(104)) {
