...
export const glyphEffects = {
  timepow: {
    ...
    effect: (level, strength) => (1.01 + Math.pow(level, 0.32) * Math.pow(strength, 0.45) / 75) * TMTE(163),
  ...
  timespeed: {
    ...
    effect: (level, strength) => ((GlyphAlteration.isEmpowered("time")
      ...
      : 1 + Math.pow(level, 0.3) * Math.pow(strength, 0.65) / 20)) * TMTE(163),
    formatEffect: x => format(x, 3, 3),
  ...
  timeetermult: {
    ...
    effect: (level, strength) => Math.pow((strength + 3) * level, 0.9) *
      Math.pow(3, GlyphAlteration.sacrificeBoost("time")) * TMTE(163),
  ...
  timeEP: {
    ...
    shortDesc: () => (GlyphAlteration.isAdded("time")
      ? "EP ×{value} and ^{value2}"
      : "EP ×{value}"),
    effect: (level, strength) => (Math.pow(level * strength, 3) * 100) * TMTE(163),
  ...
  dilationDT: {
    ...
    effect: (level, strength) => (GlyphAlteration.isEmpowered("dilation")
      ...
      : Decimal.pow(level * strength, 1.5).times(2)).times(TMTE(163)),
  ...
  dilationgalaxyThreshold: {
    ...
    effect: (level, strength) => (1 - Math.pow(level, 0.17) * Math.pow(strength, 0.35) / 100 -
      GlyphAlteration.sacrificeBoost("dilation") / 50) * TMTE(163),
    formatEffect: x => format(x, 3, 3),
  ...
  dilationTTgen: {
    ...
    shortDesc: () => (GlyphAlteration.isAdded("dilation")
      ...
      : "{value} TT/hr"),
    effect: (level, strength) => TMTE(163) * Math.pow(level * strength, 0.5) / 10000,
  ...
  dilationpow: {
    ...
    effect: (level, strength) => (1.1 + Math.pow(level, 0.7) * Math.pow(strength, 0.7) / 25) * TMTE(163),
    formatEffect: x => format(x, 2, 2),
  ...
  replicationspeed: {
    ...
    effect: (level, strength) => (GlyphAlteration.isEmpowered("replication")
      ...
      : Decimal.times(level, strength).times(3)).times(TMTE(163)),
    formatEffect: x => format(x, 2, 1),
    combine: GlyphCombiner.multiplyDecimal,
  ...
  replicationpow: {
    ...
    effect: (level, strength) => TMTE(163) * (1.1 + Math.pow(level, 0.5) * strength / 25 +
  ...
  replicationdtgain: {
    ...
    effect: (level, strength) => TMTE(163) * 0.0003 * Math.pow(level, 0.3) * Math.pow(strength, 0.65),
  ...
  replicationglyphlevel: {
    ...
    effect: (level, strength) => TMTE(163) * Math.pow(Math.pow(level, 0.25) * Math.pow(strength, 0.4), 0.5) / 50,
    formatEffect: x => format(x, 3, 3),
    combine: effects => {
  ...
  infinitypow: {
    ...
    effect: (level, strength) => TMTE(163) * (1.007 + Math.pow(level, 0.21) * Math.pow(strength, 0.4) / 75 +
  ...
  infinityrate: {
    ...
    effect: (level, strength) => TMTE(163) * Math.pow(level, 0.2) * Math.pow(strength, 0.4) * 0.04,
    formatEffect: x => format(x, 2, 2),
    combine: GlyphCombiner.add,
    enabledInDoomed: true,
  },
  infinityIP: {
    ...
    shortDesc: () => (GlyphAlteration.isAdded("infinity")
      ? "IP ×{value} and ^{value2}"
      : "IP ×{value}"),
    effect: (level, strength) => Math.pow(level * (strength + 1), 6) * 10000 * TMTE(163),
    formatEffect: x => format(x, 2, 3),
    combine: GlyphCombiner.multiply,
  ...
  powerpow: {
    ...
    shortDesc: () => (GlyphAlteration.isAdded("power")
      ...
      : "AD power +{value}"),
    effect: (level, strength) => TMTE(163) * (1.015 + Math.pow(level, 0.2) * Math.pow(strength, 0.4) / 75),
    formatEffect: x => format(x, 3, 3),
    formatSingleEffect: x => format(x - 1, 3, 3),
    combine: GlyphCombiner.addExponents,
    conversion: x => 2 / (x + 1),
  ...
  powermult: {
    ...
    effect: (level, strength) => (GlyphAlteration.isEmpowered("power")
      ...
      : Decimal.pow(level * strength * 10, level * strength * 10)).times(TMTE(163)),
  ...
  powerdimboost: {
    ...
    effect: (level, strength) => Math.pow(level * strength, 0.5) *
      Math.pow(1 + GlyphAlteration.sacrificeBoost("power"), 3) * TMTE(163),
    formatEffect: x => format(x, 2, 2),
    combine: GlyphCombiner.multiply,
  ...
  powerbuy10: {
    ...
    effect: (level, strength) => (1 + level * strength / 12) * TMTE(163),
    formatEffect: x => format(x, 2, 2),
    combine: GlyphCombiner.addExponents,
    enabledInDoomed: true,
  },
  effarigrm: {
    ...
    effect: (level, strength) => (GlyphAlteration.isEmpowered("effarig")
      ? Math.pow(level, 1.5)
      : Math.pow(level, 0.6) * strength) * (TMTE(163)),
    formatEffect: x => format(x, 2, 2),
    combine: GlyphCombiner.multiply,
  ...
  effarigglyph: {
    ...
    effect: (level, strength) => Math.floor(10 * Math.pow(level * strength, 0.5) * TMTE(163)),
  ...
  effarigblackhole: {
    ...
    effect: (level, strength) => (1 + Math.pow(level, 0.25) * Math.pow(strength, 0.4) / 75) * TMTE(163),
    formatEffect: x => format(x, 3, 3),
    formatSingleEffect: x => format(x - 1, 3, 3),
    combine: GlyphCombiner.addExponents,
  },
  effarigachievement: {
    ...
    effect: (level, strength) => (1 + Math.pow(level, 0.4) * Math.pow(strength, 0.6) / 60 +
      GlyphAlteration.sacrificeBoost("effarig") / 10) * TMTE(163),
    formatEffect: x => format(x, 3, 3),
    formatSingleEffect: x => format(x - 1, 3, 3),
    combine: GlyphCombiner.addExponents,
  ...
  effarigforgotten: {
    ...
    effect: (level, strength) => (1 + 2 * Math.pow(level, 0.25) * Math.pow(strength, 0.4)) * TMTE(163),
    formatEffect: x => format(x, 2, 2),
    combine: GlyphCombiner.multiply,
  ...
  effarigdimensions: {
    ...
    effect: (level, strength) => (1 + Math.pow(level, 0.25) * Math.pow(strength, 0.4) / 500) * TMTE(163),
    formatEffect: x => format(x, 3, 3),
    formatSingleEffect: x => format(x - 1, 3, 3),
    combine: GlyphCombiner.addExponents,
  },
  effarigantimatter: {
    ...
    effect: (level, strength) => (1 + Math.pow(level, 0.25) * Math.pow(strength, 0.4) / 5000) * TMTE(163),
  ...
  timeshardpow: {
    ...
    effect: (level, strength) => TMTE(163) * (1 + (strength / 3.5) * Math.pow(level, 0.35) / 400),
    formatEffect: x => format(x, 3, 3),
    formatSingleEffect: x => format(x - 1, 3, 3),
    combine: GlyphCombiner.addExponents,
    enabledInDoomed: true,
  ...
  cursedgalaxies: {
    ...
    effect: level => Math.pow(level, -0.03) * TMTE(163),
  ...
  curseddimensions: {
    ...
    effect: level => Math.pow(level, -0.035) * TMTE(163),
    formatEffect: x => format(x, 3, 3),
    combine: GlyphCombiner.multiply,
  },
  cursedtickspeed: {
    ...
    effect: level => Math.clampMin(Math.log10(level)*TMTE(163), 1),
    formatEffect: x => format(x, 3, 3),
    combine: GlyphCombiner.add,
  },
  cursedEP: {
    ...
    effect: level => Decimal.pow10(-level / 10).times(TMTE(163)),
  ...
  realityglyphlevel: {
    ...
    effect: level => Math.floor(Math.sqrt(level * 90 * TMTE(163))),
    formatEffect: x => formatInt(x),
    combine: GlyphCombiner.add,
  },
  realitygalaxies: {
    ...
    totalDesc: "All Galaxy strength +{value}",
    shortDesc: "Galaxy Strength +{value}",
    effect: level => TMTE(163) * (1 + Math.pow(level / 100000, 0.5)),
  ...
  realityrow1pow: {
    ...
    effect: level => TMTE(163) * (1 + level / 125000),
    formatEffect: x => format(x, 3, 3),
    combine: GlyphCombiner.addExponents,
  },
  realityDTglyph: {
  ...
  companiondescription: {
    ...
    effect: () => {
      ...
      if (cursedCount > 0) return Math.pow(0.2 + 0.2 * Math.random(), cursedCount) * TMTE(163);
      return 0.4 + 0.6 * Math.random() * TMTE(163);
    },
  ...
  companionEP: {
    ...
    effect: (level, strength) => Decimal.pow10(1e6 * strengthToRarity(strength)).times(TMTE(163)),
  ...
  tmtrainereffect: {
    ...
    bitmaskIndex: 28,
    isGenerated: false,
    glyphTypes: ["tmtrainer"],
    singleDesc: window.RANDOM_TMTRAINER_EFFECTS(3, 2e9, true),
    shortDesc: window.RANDOM_TMTRAINER_EFFECTS(10, 1.6e9),
    totalDesc: () => window.RANDOM_TMTRAINER_EFFECTS(3, 1.2e9),
    effect: () => 0,
    formatEffect: x => formatPostBreak(x, 2),
    combine: GlyphCombiner.add,
    enabledInDoomed: true,
