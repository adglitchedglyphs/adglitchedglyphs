export const glyphSacrifice = {
  "power": {
    ...
    effect: added => {
      ...
      return Math.floor(750 * Math.pow(base, 1.2) * TMTE(164));
    },
    description: amount => {
      ...
      const nextGalaxyText = amount < 750
        ...
        : "";
      return `Distant Galaxy scaling starts ${formatInt(amount * TMTE(164))} later${nextGalaxyText}`;
    },
  ...
  "infinity": {
    ...
    effect: added => {
      ...
      const capped = Math.clampMax(sac, GlyphSacrificeHandler.maxSacrificeForEffects);
      return (1 + Math.log10(1 + Math.pow(capped, 0.2) / 100)) * TMTE(164);
    },
  ...
  "time": {
    ...
    effect: added => {
      ...
      const capped = Math.clampMax(sac, GlyphSacrificeHandler.maxSacrificeForEffects);
      return Math.pow(1 + Math.pow(capped, 0.2) / 100, 2) * TMTE(164);
    },
  ...
  "replication": {
    ...
    effect: added => {
      ...
      const capped = Math.clampMax(sac, GlyphSacrificeHandler.maxSacrificeForEffects);
      const base = Math.log10(capped + 1) / Math.log10(GlyphSacrificeHandler.maxSacrificeForEffects);
      return Math.floor(1500 * Math.pow(base, 1.2) * TMTE(164));
    },
    description: amount => {
      const sacCap = GlyphSacrificeHandler.maxSacrificeForEffects;
  ...
  "dilation": {
    ...
    effect: added => {
      ...
      return Math.pow(Math.clampMin(capped, 1), exponent) * TMTE(164);
    },
  ...
  "effarig": {
    ...
    effect: added => {
      ...
      return 2 * Math.log10(capped / 1e20 + 1) * TMTE(164);
    },
  ...
  "reality": {
    ...
    effect: added => {
      ...
      return Math.clampMax(1 + Math.sqrt(sac) / 15, 100) * TMTE(164);
    },
