...
export const breakInfinityUpgrades = {
  totalAMMult: {
    ...
    effect: () => Math.pow(player.records.totalAntimatter.exponent + 1, 0.5) * TMTE(161),
  ...
  currentAMMult: {
    ...
    effect: () => Math.pow(Currency.antimatter.exponent + 1, 0.5) * TMTE(161),
    formatEffect: value => formatX(value, 2, 2)
  },
  galaxyBoost: {
    id: "postGalaxy",
    cost: 5e11,
    description: () => `All Galaxies are ${formatPercents(0.5*TMTE(161))} stronger`,
    effect: () => 1.5*TMTE(161)
  },
  infinitiedMult: {
    id: "infinitiedMult",
    ...
    effect: () => 1 + Currency.infinitiesTotal.value.pLog10() * 10 * TMTE(161),
    formatEffect: value => formatX(value, 2, 2)
  },
  achievementMult: {
    id: "achievementMult",
    ...
    effect: () => Math.max(Math.pow((Achievements.effectiveCount - 30), 3) / 40, 1)*TMTE(161),
    formatEffect: value => formatX(value, 2, 2)
  },
  slowestChallengeMult: {
    ...
    effect: () => Decimal.clampMin(50 / Time.worstChallenge.totalMinutes, 1).times(TMTE(161)),
    formatEffect: value => formatX(value, 2, 2),
    hasCap: true,
    cap: DC.D3E4
  },
  infinitiedGen: {
    ...
    effect: () => player.records.bestInfinity.time*(1/TMTE(161)),
    formatEffect: value => {
  ...
  ipGen: rebuyable({
    ...
    costIncrease: 10,
    maxUpgrades: 10,
    effect: value => Player.bestRunIPPM.times(value / 20).times(TMTE(161)),
    description: () => {
