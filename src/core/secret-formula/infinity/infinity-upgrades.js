...
function dimInfinityMult() {
  return Currency.infinitiesTotal.value.times(0.2).plus(1).times(TMTE(162));
}
function chargedDimInfinityMult() {
  return (1 + Math.log10(Math.max(1, Currency.infinitiesTotal.value.pLog10())) * Math.sqrt(Ra.pets.teresa.level) / 150) * TMTE(162);
}
...
export const infinityUpgrades = {
  totalTimeMult: {
    ...
    effect: () => Math.pow(Time.totalTimePlayed.totalMinutes / 2, 0.15) * TMTE(162),
    ...
    charged: {
      ...
      effect: () => (1 +
        ...
        Math.pow(Ra.pets.teresa.level, 0.5) / 150) * TMTE(162),
  ...
  buy10Mult: {
    ...
    charged: {
      ...
      effect: () => (1 + Ra.pets.teresa.level / 200) * TMTE(162),
  ...
  galaxyBoost: {
    ...
    charged: {
      ...
      effect: () => (2 + Math.sqrt(Ra.pets.teresa.level) / 100) * TMTE(162),
  ...
  thisInfinityTimeMult: {
    ...
    effect: () => Decimal.max(Math.pow(Time.thisInfinity.totalMinutes / 4, 0.25), 1).times(TMTE(162)),
    formatEffect: value => formatX(value, 2, 2),
    charged: {
      description:
      ...
      effect: () => (1 +
        ...
        Math.sqrt(Ra.pets.teresa.level) / 150) * TMTE(162),
      formatEffect: value => formatPow(value, 4, 4)
    }
  },
  unspentIPMult: {
    ...
    effect: () => Currency.infinityPoints.value.dividedBy(2).pow(1.5).plus(1).times(TMTE(162)),
    formatEffect: value => formatX(value, 2, 2),
    charged: {
      description: "Multiplier to 1st Antimatter Dimension based on unspent Infinity Points, powered by Teresa level",
      effect: () => Currency.infinityPoints.value.dividedBy(2).pow(Math.sqrt(Ra.pets.teresa.level) * 1.5).plus(1).times(TMTE(162)),
      formatEffect: value => formatX(value, 2, 2)
    }
  },
  dimboostMult: {
  ...
  ipOffline: {
    ...
    effect: () => (player.options.offlineProgress
      ? player.records.thisEternity.bestIPMsWithoutMaxAll.times(TimeSpan.fromMinutes(1).totalMilliseconds / 2).times(TMTE(162))
      : DC.D0),
  ...
  ipMult: {
    ...
    effect: () => (player.IPMultPurchases >= 3300000 ? DC.E1E6 : DC.D2.pow(player.IPMultPurchases)).times(TMTE(162)),
