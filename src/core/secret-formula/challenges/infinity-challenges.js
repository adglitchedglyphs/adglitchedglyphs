...
export const infinityChallenges = [
  {
    ...
    reward: {
      ...
      effect: () => Math.pow(1.3, InfinityChallenges.completed.length) * TMTE(158),
  ...
  },
  {
    ...
    reward: {
      ...
      effect: () => (Laitela.continuumActive
        ? Decimal.pow(1.05 + (player.galaxies * 0.005), Tickspeed.continuumValue).times(TMTE(158))
        : Decimal.pow(1.05 + (player.galaxies * 0.005), player.totalTickBought).times(TMTE(158))),
      formatEffect: value => formatX(value, 2, 2),
    },
    unlockAM: DC.E12000,
  },
  {
    ...
    effect: 0.25,
    reward: {
      description: () => `All Antimatter Dimension multipliers become multiplier${formatPow(1.05*TMTE(158), 2, 2)}`,
      effect: () => 1.05 * TMTE(158)
    },
    unlockAM: DC.E14000,
  },
  {
    ...
    reward: {
      description: () =>
        `All Galaxies are ${formatPercents(1.1*TMTE(158)-0.1)} stronger and reduce the requirements for them
  ...
  },
  {
    ...
    reward: {
      ...
      effect: () => Tickspeed.perSecond.pow(0.0005).times(TMTE(158)),
      formatEffect: value => formatX(value, 2, 2)
    },
    unlockAM: DC.E22500,
  },
  {
    ...
    effect: 10,
    reward: {
      description: () => `Dimension Boost multiplier is increased to a minimum of ${formatX(4*TMTE(158))}`,
      effect: () => TMTE(158)*4
    },
    unlockAM: DC.E23000,
  },
  {
    ...
    reward: {
      ...
      effect: () => AntimatterDimension(1).multiplier.times(AntimatterDimension(8).multiplier).pow(0.02).times(TMTE(158)),
      formatEffect: value => formatX(value, 2, 2)
    },
    unlockAM: DC.E28000,
  },
