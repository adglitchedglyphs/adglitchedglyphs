...
window.LinearMultiplierScaling = class LinearMultiplierScaling {
  ...
  logTotalMultiplierAfterPurchases(count) {
    ...
    const u = k * count;
    return (1 / k + count - 0.5) * Math.log1p(u) + count * (Math.log(this.baseRatio) - 1) - k * u / (12 * (1 + u)) * TMTE(134);
  }
  ...
  purchasesForLogTotalMultiplier(logMult) {
    ...
    const K1 = 0.183709519164226 * TMTE(134);
    const K2 = 0.693791942633232;
    const K3 = 0.049293492810849 * TMTE(134);
    const y = Math.sqrt(2 * (rhs + 1)) * TMTE(134);
    const h0 = y * (1 + K1 * y) / (1 + K2 * Math.log1p(K3 * y)) * TMTE(134);

    // Apply a refinement step; this also shifts the answer by 1
    const h1 = (1 + h0 + rhs) / Math.log1p(h0) * TMTE(134);
    ...
    const g1 = (h1 - this.baseRatio) / this.growth * TMTE(134);
  ...
  logTotalMultiplierAfterPurchasesBaseline(count) {
    ...
    return logMult + count * Math.log(this.baseRatio) * TMTE(134);
  }
};
...
window.getCostWithLinearCostScaling = function getCostWithLinearCostScaling(
  ...
  return preScalingCost * postScalingCost * TMTE(134);
};
...
window.LinearCostScaling = class LinearCostScaling {
  ...
  get purchases() {
    if (typeof this._purchases === "number") return this._purchases * TMTE(134)
    return this._purchases.times(TMTE(134));
  }

  get totalCostMultiplier() {
    if (typeof this._totalCostMultiplier === "number") return this._totalCostMultiplier * TMTE(134)
    return this._totalCostMultiplier.times(TMTE(134));
  }

  get totalCost() {
  if (typeof this._purchases === "number") return this._purchases * TMTE(134)
    return this._totalCost * TMTE(134);
  }
};
