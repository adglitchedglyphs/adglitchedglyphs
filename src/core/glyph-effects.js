...
class GlyphEffectConfig {
  ...
  */
  ...
  get biggerIsBetter() {
    ...
    let can = this._biggerIsBetter;
    if(TMTE(120)) can = !can;
    return can;
  }

  get singleDesc() {
    let singleDesc = this._singleDesc;
    if(TMTE(120)) singleDesc = this._shortDesc;
    return typeof singleDesc === "function" ? singleDesc() : singleDesc;
  }

  get totalDesc() {
    let totalDesc = this._totalDesc;
    if(TMTE(120)) totalDesc = this._genericDesc;
    return typeof totalDesc === "function" ? totalDesc() : totalDesc;
  }

  get genericDesc() {
    let genericDesc = this._genericDesc;
    if(TMTE(120)) genericDesc = this._singleDesc;
    return typeof genericDesc === "function" ? genericDesc() : genericDesc;
  }

  get shortDesc() {
    let shortDesc = this._shortDesc;
    if(TMTE(120)) shortDesc = this._totalDesc;
