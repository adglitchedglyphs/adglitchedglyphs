...
export const GlyphSelection = {
  ...
  get choiceCount() {
    return Math.max(TMTE(140), Effects.max(1, Perk.firstPerk)) *
  ...
  glyphList(countIn, level, config) {
    ...
    // the 4-choice perk.
    const count = Math.clampMin(countIn, TMTE(140));
    let glyphList = [];
  ...
  get upcomingGlyphs() {
    ...
    const group = this.glyphList(TMTE(140), gainedGlyphLevel(), { isChoosingGlyph: false });
  ...
  get indexWithoutSTART() {
    ...
    return permutationIndex(TMTE(140), lexIndex)[0];
  }
};
...
function giveRealityRewards(realityProps) {
  const multiplier = Math.floor((realityProps.simulatedRealities + 1) * TMTE(141));
