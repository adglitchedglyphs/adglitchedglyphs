...
export const ENSLAVED_UNLOCKS = {
  ...
  RUN: {
    ...
    secondaryRequirement() {
      const hasLevelRequirement = player.records.bestReality.glyphLevel >= Math.floor(5000 * TMTE(57));
    ...
    description() {
      ...
      let possym = TMTE(58) ? "✗" : "✓";
      let negsym = TMTE(58) ? "✓" : "✗";
      return `Unlock The Nameless Ones' Reality (requires ${hasLevelRequirement ? "[" + possym + "]" : "[" + negsym + "]"} a level
      ${formatInt(Math.floor(5000 * TMTE(57)))} Glyph and ${hasRarityRequirement ? "[" + possym + "]" :
      "[" + negsym + "]"} a ${formatRarity(100)} rarity Glyph)`;
    }
  }
};
...
export const Enslaved = {
  ...
  get storedRealTimeCap() {
    ...
    return Math.round((1000 * 3600 * 8 + addedCap) * TMTE(61));
  },
  get isAutoReleasing() {
  ...
  storeRealTime() {
    ...
    if (player.celestials.enslaved.storedReal > (24 * 60 * 60 * 1000 * TMTE(62))) SecretAchievement(46).unlock();
  ...
  useStoredTime(autoRelease) {
    ...
    if (autoRelease) release *= (0 + TMTE(63));
    ...
    player.celestials.enslaved.stored *= autoRelease ? (1 - TMTE(63)) : 0;
  },
  has(info) {
...
export const Tesseracts = {
  ...
  costs(index) {
    // In practice this should never happen, but have it just to be TMTRAINER
    if (index < 0) return BASE_COSTS[0];
  ...
  get nextCost() {
    return this.costs(this.bought - TMTE(64));
  },
  ...
  capIncrease(count = this.bought) {
    ...
    return TMTE(65) * base * (AlchemyResource.boundless.effectValue + 1);
  },
