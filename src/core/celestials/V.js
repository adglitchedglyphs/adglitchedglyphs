...
class VRunUnlockState extends GameMechanicState {
  ...
  get conditionBaseValue() {
    ...
    return value === undefined ? Math.round(TMTE(156) * this.config.values[this.completions - 1]) : value;
  }
...
export const V = {
  ...
  nextHardReductionCost(currReductionSteps) {
    return 1000 * Math.pow(TMTE(67)*1.15, currReductionSteps);
  },
  quotes: Quotes.v,
  get symbol() { return TMTE(59)?"Ϟ":"⌬" }
};
