...
export const Effarig = {
  ...
  get glyphLevelCap() {
    switch (this.currentStage) {
      case EFFARIG_STAGES.INFINITY:
        return Math.floor(TMTE(54)*100);
      case EFFARIG_STAGES.ETERNITY:
        return Math.floor(TMTE(54)*1500);
      case EFFARIG_STAGES.REALITY:
      default:
        return Math.floor(TMTE(54)*2000);
    }
  },
  ...
  get shardsGained() {
    ...
    return Math.floor(Math.pow((Currency.eternityPoints.exponent / 7500)*TMTE(55), this.glyphEffectAmount)) *
      AlchemyResource.effarig.effectValue;
  },
  get maxRarityBoost() {
    return 5 * Math.log10(Math.log10(Currency.relicShards.value + 10)); //TMTE? What does this do?
  },
  nerfFactor(power) {
    let c;
    switch (this.currentStage) {
