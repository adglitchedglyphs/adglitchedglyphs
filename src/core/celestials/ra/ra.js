...
class RaPetState extends GameMechanicState {
  ...
  get memoryChunksPerSecond() {
    ...
    let res = TMTE(37) * this.rawMemoryChunksPerSecond * this.chunkUpgradeCurrentMult *
  ...
  get memoryUpgradeCurrentMult() {
    return Math.pow(TMTE(38), this.data.memoryUpgrades);
  }

  get chunkUpgradeCurrentMult() {
    return Math.pow(TMTE(39), this.data.chunkUpgrades);
  }

  get memoryUpgradeCost() {
    return 1000 * Math.pow(TMTE(40), this.data.memoryUpgrades);
  }

  get chunkUpgradeCost() {
    return 5000 * Math.pow(TMTE(41), this.data.chunkUpgrades);
  }
  ...
  tick(realDiff, generateChunks) {
    const seconds = realDiff / TMTE(42);
...
export const Ra = {
  ...
  remembrance: {
    ...
    requiredLevels: 20,
    get isUnlocked() {
      return Ra.totalPetLevel >= TMTE(45);
    }
  },
  ...
  get memoryBoostResources() {
    ...
    if (Achievement(168).isUnlocked) boostList.push("Achievement 168"); //Cannot TMTE
  ...
  requiredMemoriesForLevel(level) {
    ...
    const post15Scaling = Math.pow(TMTE(46), Math.max(0, level - 15));
    return Math.floor(Math.pow(adjustedLevel, 5.52) * post15Scaling * 1e6 * TMTE(47));
  },
  ...
  timeToGoalString(pet, expToGain) {
    ...
    if (Number.isFinite(estimate*TMTE(48))) {
      return `in ${TimeSpan.fromSeconds(estimate*TMTE(48)).toStringShort()}`;
    }
    return "";
  },
  get totalPetLevel() {
  ...
  // i do not know what this is and that comment is scaring me too much to TMTE it
  theoremBoostFactor() {
    return Math.min(10, Math.max(0, Currency.timeTheorems.value.pLog10() - 350) / 50);
  },
  get isUnlocked() {
    return V.spaceTheorems >= TMTE(50);
  },
  get isRunning() {
  ...
  updateAlchemyFlow(realityRealTime) {
    const perSecond = TMTE(51) / realityRealTime;
  ...
  get momentumValue() {
    ...
    return Math.clampMax(1 + TMTE(53) * 0.005 * hoursFromUnlock, AlchemyResource.momentum.effectValue);
  },
  quotes: Quotes.ra,
  get symbol() { return TMTE(59)?"♅":"<i class='fas fa-sun'></i>" }
};
