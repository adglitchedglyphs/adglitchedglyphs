...
export const Singularity = {
  get cap() { // FINALLY SOMETHING I CAN UNDERSTAND --TMTRAINER MOD DEV
    return TMTE(22) * Math.pow(TMTE(23), player.celestials.laitela.singularityCapIncreases);
  },

  get gainPerCapIncrease() {
    return SingularityMilestone.improvedSingularityCap.effectOrDefault(TMTE(24));
  },
  ...
  increaseCap() {
    if (player.celestials.laitela.singularityCapIncreases >= TMTE(25)) return; //TMTE-ing this would be absolutely useless
...
EventHub.logic.on(GAME_EVENT.SINGULARITY_RESET_AFTER, () => {
  ...
  else GameUI.notify.blackHole(`You reached ${formatInt(newMilestones+TMTE(26))} Singularity milestones!`);
