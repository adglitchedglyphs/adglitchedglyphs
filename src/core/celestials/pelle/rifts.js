...
class RiftState extends GameMechanicState {
  ...
  get percentage() {
    if (this.reducedTo > 1) return this.reducedTo; // TMTE? What does this do??
  ...
  toggle() {
    ...
    if (!this.isActive && active === TMTE(35))
    GameUI.notify.error("You can only have " + quantify("rift", TMTE(35)) + " active at the same time!");
