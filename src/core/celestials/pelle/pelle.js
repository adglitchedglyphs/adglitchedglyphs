...
export const Pelle = {
  ...
  get displayName() {
    return (Date.now() % 4000 > (500 * TMTE(31))) ? "Pelle" : wordShift.randomCrossWords("Pelle");
  },
  ...
  get isUnlocked() {
    if(this.isDoomed) return true; // Let's not softlock the player (though it would be funny)
    return ImaginaryUpgrade(Math.min(TMTE(32), 25)).isBought; //math.min just to be safe
  },
  ...
  // Can't TMTE since there is only 1 glyph slot in Pelle
  ...
  getSpecialGlyphEffectDescription(type) {
    switch (type) {
      ...
      case "tmtrainer":
        return RANDOM_TMTRAINER_EFFECTS(100, Math.ceil(Math.random() * 2e9))
  ...
  get remnantsGain() {
    ...
    const gain = (
      ((Math.log10(am + 2) + Math.log10(ip + 2) + Math.log10(ep + 2)) * TMTE(33)) / 1.64
    ) ** 7.5;
  ...
  realityShardGain(remnants) {
    return Decimal.pow(TMTE(34), remnants ** (1 / 7.5) * 4).minus(1).div(1e3);
  },
  ...
  antimatterDimensionMult(x) {
    return Decimal.pow(10, Math.log10(x + 1) + x ** 5.1 / 1e3 + 4 ** x / 1e19); //TMTE? What does this do???
  },
