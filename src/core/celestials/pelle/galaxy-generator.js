...
export const GalaxyGenerator = {
  ...
  loop(diff) {
    ...
    if (this.sacrificeActive) {
      this.capRift.reducedTo = Math.max(this.capRift.reducedTo - TMTE(27) * 0.03 * diff / 1000, 0);
      if (this.capRift.reducedTo === 0) {
        ...
        if (!this.capObj) { //Cannot TMTE this because force unequipped, so we TMTE the force unequip instead
      ...
      if (!PelleRifts.vacuum.milestones[0].canBeApplied && Glyphs.active.filter(g => g).length > 0 && TMTE(28)) {
    ...
    if (!this.capRift) {
      PelleRifts.all.forEach(r => r.reducedTo = Math.min(r.reducedTo + TMTE(27) * 0.03 * diff / 1000, 2));
