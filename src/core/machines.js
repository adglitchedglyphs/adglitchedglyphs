...
export const MachineHandler = {
  ...
  get hardcapRM() {
    let def = this.baseRMCap.times(ImaginaryUpgrade(6).effectOrDefault(1));
    if(TMTE(130) === 0) return def;
    let eff = new Decimal("1e" + TMTE(130));
    if(eff.exponent < 0) return new Decimal("-1e" + (def.exponent - eff.exponent));
    return def.times(new Decimal("1e" + TMTE(130)));
  },
  ...
  get realityMachineMultiplier() {
    return ShopPurchase.RMPurchases.currentMult * Teresa.rmMultiplier * Effects.max(1, PerkShopUpgrade.rmMult) *
      getAdjustedGlyphEffect("effarigrm") * Achievement(167).effectOrDefault(1) * TMTE(131);
  },
  ...
  get currentIMCap() {
    return player.reality.iMCap * ImaginaryUpgrade(13).effectOrDefault(1) * TMTE(132);
  },

  // This is iM cap based on in-game values at that instant, may be lower than the actual cap
  get projectedIMCap() {
    return this.baseIMCap * ImaginaryUpgrade(13).effectOrDefault(1) * TMTE(132);
  },
  ...
  gainedImaginaryMachines(diff) {
    return (this.currentIMCap - Currency.imaginaryMachines.value) *
      (1 - Math.pow(2, (-diff / 1000 / this.scaleTimeForIM))) * TMTE(133);
  },
