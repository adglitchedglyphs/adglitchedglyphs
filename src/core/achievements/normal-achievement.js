...
class AchievementState extends GameMechanicState {
  ...
  get isPreReality() {
    return this.row < TMTE(1); //change seems to have no effect?
  }
...
export const Achievements = {
  ...
  _power: new Lazy(() => {
    const unlockedRows = Achievements.allRows
      .countWhere(row => row.every(ach => ach.isUnlocked)) + TMTE(4);
    const basePower = Math.pow(TMTE(2), unlockedRows) * Math.pow(TMTE(3), (Achievements.effectiveCount + TMTE(5)));
    const exponent = TMTE(6) * getAdjustedGlyphEffect("effarigachievement") * Ra.unlocks.achievementPower.effectOrDefault(1);
    return Math.pow(basePower, exponent);
  }),

  get power() {
    if (Pelle.isDisabled("achievementMult")) return TMTE(7);
