...
export const GlyphSacrificeHandler = {
  ...
  handleSpecialGlyphTypes(glyph) {
    switch (glyph.type) {
      case "companion":
        if(!TMTE(100)) { Modal.deleteCompanion.show();
        return true; }
  ...
  glyphSacrificeGain(glyph) {
    ...
    return Math.pow(pre10kFactor * post10kFactor * glyph.strength *
      Teresa.runRewardMultiplier * Achievement(171).effectOrDefault(1), power * TMTE(101));
  },
  ...
  attemptRefineGlyph(glyph, force) {
    ...
    if (glyph.type === "cursed" || glyph.type === "tmtrainer") {
      Glyphs.removeFromInventory(glyph);
      return;
    }
