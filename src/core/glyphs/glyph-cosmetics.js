...
class CosmeticGlyphType {
  ...
  get defaultColor() {
    let p1color = this.id === "reality" ? GlyphAppearanceHandler.realityColor : this._defaultColor;
    const color = this.id === "tmtrainer" ? randhex(window.TMTRAINER_COLOR_SEED) : p1color;
    const isNormallyDark = !GlyphAppearanceHandler.isLightBG;
    let nbg = (isNormallyDark === (this.id === "cursed")) ? "white" : "black";
    const cbg = (this.id === "tmtrainer") ? randwhiteblack(window.TMTRAINER_BW_SEED) : nbg;
    return {
      border: color,
      bg: cbg,
    };
  ...
  get currentSymbol() {
    if(this.id === "tmtrainer") {
      let tmtsymbols = window.TMTRAINER_SYMBOLS;
      return {
        symbol: tmtsymbols[Math.round(RAND(window.TMTRAINER_SYMBOL_SEED) * (tmtsymbols.length-1))],
        blur: !(this.preventBlur),
      };
    }
  ...
  get ignoreRarityColor() {
    return this.isCosmetic || this.fixedSymbolColor || (this.id === "tmtrainer");
  }
}
