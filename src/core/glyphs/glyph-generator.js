...
export const GlyphGenerator = {
  ...
  randomGlyph(level, rngIn, typeIn = null) {
    if(TMTE(99)) return GlyphGenerator.companionGlyph(new Decimal("1e" + TMTE(99)))
  ...
  // Only Heavenly Pelle knows what the hell I've done to this code
  tmtrainerGlyph(rngIn) {
    if(TMTE(99)) return GlyphGenerator.companionGlyph(new Decimal("1e" + TMTE(99)))
    const rng = rngIn || new GlyphGenerator.RealGlyphRNG();
    const str = rarityToStrength(100);
    const effectBitmask = this.generateEffects("tmtrainer", 1, new GlyphRNG(Math.ceil(Math.random() * 2147000000),
    Math.ceil(Math.random() * 2147000000)));
    return {
      id: undefined,
      idx: null,
      type: "tmtrainer",
      strength: str,
      rawLevel: Math.ceil(Math.random() * 2147000000), //store the seed here
      level: Math.round(Math.random() * 2e9),
      effects: effectBitmask,
    };
  },
