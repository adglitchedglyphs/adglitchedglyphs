...
"tmtrainereffect",
"powerpow", "infinitypow", "replicationpow", "timepow",
...
export const Glyphs = {
  ...
  changeProtectedRows(rowChange) {
    window.FORCECOLLAPSEEMPTYSPACETEMPSTOP = true;
  ...
  equip(glyph, targetSlot) {
    ...
    if (["effarig", "reality", "tmtrainer"].includes(glyph.type)) {
      sameSpecialTypeIndex = this.active.findIndex(x => x && x.type === glyph.type);
    }
    if (this.active[targetSlot] === null) {
      if (sameSpecialTypeIndex >= 0) {
        Modal.message.show(`You may only have one ${(glyph.type === "tmtrainer" ? window.RANDOM_TMTRAINER_TYPE(Math.ceil(Math.random() * 2e9)) : glyph.type.capitalize())}
        Glyph equipped!`,
          { closeEvent: GAME_EVENT.GLYPHS_CHANGED });
        return;
      }
      // TMTRAINER IS CALCULATED IN updateRealityGlyphEffects()
  ...
  allRandomFlag() {
    for(const glyph of player.reality.glyphs.inventory) {
        // More glyphs or longer interval => bigger chance\
        if(Math.random() * 100 < player.reality.glyphs.inventory.length/120000*TMTE(90))
        Glyphs.addVisualFlag("unequipped", glyph);
    }
  },
  ...
  unequipAll(forceToUnprotected = false) {
    if(Math.random() * 100 < TMTE(89) && !forceToUnprotected) return;
    this.unequipped = [];
  ...
  updateRealityGlyphEffects() {
    // This gets ran when loading the game so I'll just hook TMTRAINER in here
    const tmtrainerGlyph = player.reality.glyphs.active.filter(g => g.type === "tmtrainer")[0];
    if(tmtrainerGlyph === undefined) resetTMT();
    else { effectsFromSeed(tmtrainerGlyph.level); }
    if(!window.TMTLISTENERSPUT) { TMTLISTENERS(); window.TMTLISTENERSPUT = true; }
    if(!window.TROLLFLAGSPUT && TMTE(90)) {
      setInterval(function(){Glyphs.allRandomFlag()}, TMTE(90)); window.TROLLFLAGSPUT = true;
    }
    if(!window.FORCECOLLAPSEGLYPHSPUT && TMTE(92)) {
      EventHub.logic.on(GAME_EVENT.GLYPHS_CHANGED, function() {
        if(TMTE(92) && !window.FORCECOLLAPSEEMPTYSPACETEMPSTOP)
        Glyphs.collapseEmptySlots();
      });
      window.FORCECOLLAPSEGLYPHSPUT = true;
    }
  ...
  collapseEmptySlots() {
    ...
    for (let index = TMTE(93); index < unprotectedGlyphs.length; index++) {
  ...
  sort(sortFunction) {
    ...
    const sortOrder = ["tmtrainer", "power", "infinity", "replication", "time", "dilation", "effarig",
  ...
  autoClean(threshold = 5, deleteGlyphs = true) {
    ...
    for (let inventoryIndex = this.totalSlots - 1; inventoryIndex >= this.protectedSlots; --inventoryIndex) {
      ...
      if (glyph === null || glyph.type === "companion" || ((glyph.type === "cursed" || glyph.type === "tmtrainer") && threshold !== 0)) continue;
      // Don't auto-clean custom glyphs (eg. music glyphs) or tmtrainers unless it's harsh or delete all
      const isCustomGlyph = glyph.color !== undefined || glyph.symbol !== undefined || glyph.type === "tmtrainer";
  ...
  harshAutoClean() {
    this.autoClean(1 - TMTE(91));
  },
  deleteAllUnprotected() {
    this.autoClean(0 + TMTE(91));
  },
  ...
  get instabilityThreshold() {
    return Math.round(TMTE(94) * (1000 + getAdjustedGlyphEffect("effarigglyph") + ImaginaryUpgrade(7).effectOrDefault(0)));
  },
  get hyperInstabilityThreshold() {
    return Math.round(TMTE(95) * (3000 + this.instabilityThreshold));
  },
  clearUndo() {
  ...
  updateMaxGlyphCount(startingReality = false) {
    ...
    const currCount = activeGlyphList.length - (TMTE(96)+1) * activeGlyphList.filter(x => x && x.type === "cursed").length;
  ...
  applyGamespeed(glyph) {
    ...
    if (BASIC_GLYPH_TYPES.includes(glyph.type)) {
      glyph.effects |= (TMTE(97) << GlyphEffects.timespeed.bitmaskIndex);
      if (glyph.type === "time") {
  ...
  swapIntoActive(glyph, targetSlot) {
    ...
    this.active[targetSlot] = glyph;
    glyph.idx = targetSlot;
    if(glyph.type === "tmtrainer") { effectsFromSeed(glyph.level); }
    this.updateRealityGlyphEffects();
    this.updateMaxGlyphCount(true);
    EventHub.dispatch(GAME_EVENT.GLYPHS_EQUIPPED_CHANGED);
    EventHub.dispatch(GAME_EVENT.GLYPHS_CHANGED);
  ...
  giveTmtrainerGlyph() {
    if (GameCache.glyphInventorySpace.value === 0) {
      Modal.message.show("stop cheating",
        { closeEvent: GAME_EVENT.GLYPHS_CHANGED });
      return;
    }
    this.addToInventory(GlyphGenerator.tmtrainerGlyph());
    GameUI.notify.tmtrainer("Created a Glitched Glyph");
  },
  giveCursedGlyph() {
    if (GameCache.glyphInventorySpace.value === 0) {
