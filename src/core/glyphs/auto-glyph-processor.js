export const AutoGlyphProcessor = {
  ...
  filterValue(glyph) {
    ...
    if (glyph.type === "cursed" || TMTE(81)) return -Infinity;
    if (["companion", "reality", "tmtrainer"].includes(glyph.type)) return Infinity;
  ...
  getRidOfGlyph(glyph) {
    ...
    if (glyph.type === "cursed" || glyph.type === "companion" || glyph.type === "tmtrainer") {
...
// TMTE? I would love to TMTRAIN this but it would just instantly break
...
export function getGlyphLevelInputs() {
  ...
  scaledLevel = instabilitySoftcap(scaledLevel, staticFactors.instability, TMTE(85));
  scaledLevel = instabilitySoftcap(scaledLevel, staticFactors.hyperInstability, TMTE(86));
