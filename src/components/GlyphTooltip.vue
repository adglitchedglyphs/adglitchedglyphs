...
export default {
  ...
  computed: {
    ...
    baseColor() {
      return (this.type === "tmtrainer") ?
      window.randlighthex(this.level)
      : CosmeticGlyphTypes[this.type].currentColor.bg;
    },
    textColor() {
    ...
    descriptionStyle() {
      ...
      return {
        ...
        animation: (this.type === "tmtrainer") ? "a-tmtrainer-glyph-name-cycle 0.5s infinite" : (this.type === "reality" ? "a-reality-glyph-name-cycle 10s infinite" : undefined),
      };
    },
    description() {
      const glyphName = `${this.type.capitalize()}`;
      const tmtrainerName = window.RANDOM_TMTRAINER_NAME(this.level);
      switch (this.type) {
        case "tmtrainer":
          return `${tmtrainerName}`;
        case "companion":
    ...
    levelText() {
      if (this.type === "tmtrainer") return "Level: " + window.RANDOM_TMTRAINER_LEVEL(RAND(this.level * 141932129));
    ...
    glyphTooltipStyle() {
      ...
      return {
        ...
        animation: (this.type === "tmtrainer") ? "a-tmtrainer-glyph-name-cycle 0.9s infinite" : (this.type === "reality" ? "a-reality-glyph-name-cycle 10s infinite" : undefined),
        color: this.textColor,
        background: this.baseColor
      };
    },
    glyphHeaderStyle() {
      const isCursed = this.type === "cursed" || this.type === "tmtrainer";
  ...
  methods: {
    ...
    sacrificeText() {
      if (this.type === "tmtrainer") return "Sacrifice: " + window.RANDOM_TMTRAINER_LEVEL(RAND(this.level * 1.5e9));
    ...
    refineText() {
      if (this.type === "tmtrainer") return "Refine: " + window.RANDOM_TMTRAINER_LEVEL(RAND(this.level * 2e9));
    ...
    scoreText() {
      if (this.type === "companion" || this.type === "cursed" || this.type === "reality" || this.type === "tmtrainer") return "";
