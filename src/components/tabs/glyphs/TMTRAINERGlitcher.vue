...
export default {
  ...
  methods: {
    ...
    openModal(glyph) {
        ...
        if(window.TMTRAINER_TRY()) {
          Glyphs.giveTmtrainerGlyph();
        }
    },

    getType(glyph) {
      return (glyph.type === "tmtrainer") ? window.RANDOM_TMTRAINER_TYPE(glyph.level) : glyph.type.capitalize();
    }
  }
};
</script>
  ...
  >
    ...
    >
      ...
      <div class="c-glyph-info-section c-cosmetic-text">
        <u>Attributes</u>
        Type: {{ this.getType(glyph) }}
      </div>
      <div class="c-glyph-info-section">
        <PrimaryButton
          class="o-primary-btn--subtab-option"
          @click="openModal(glyph)"
        >
          Glitch
        </PrimaryButton>
        <PrimaryButton
    ...
    <div v-else>
      Drag a Glyph over this box to attempt to glitch it. Glitched Glyphs may be rerolled this way. Removing, gaining, or moving any Glyphs will clear this box.
    </div>
  </div>
</template>
