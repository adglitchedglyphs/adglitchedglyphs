...
export default {
  ...
  computed: {
    ...
    uniqueGlyphText() {
      if (!this.hasEffarig && !this.hasReality && !this.hasTmtrainer) return "";
      const uniqueGlyphs = [];
      ...
      if (this.hasTmtrainer) uniqueGlyphs.push(
        `<span style="animation: a-tmtrainer-glyph-name-cycle 0.5s infinite">
         ${window.RANDOM_TMTRAINER_TYPE(Math.random() * 1.15e9)}</span>`);
  ...
  methods: {
    update() {
      ...
      this.hasTmtrainer = Glyphs.active.some(g => g && g.type === "tmtrainer");
...
<template>
  <div class="c-current-glyph-effects l-current-glyph-effects">
    ...
    <br v-if="isSoftcapActive || hasEffarig || hasReality || hasTmtrainer">
