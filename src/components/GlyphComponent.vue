...
export default {
  ...
  computed: {
    ...
    colorObj() {
      ...
      if (this.glyph.cosmetic) {
        if (this.glyph.cosmetic === this.glyph.type) {
          overrideColor = (this.glyph.type === "cursed" || this.glyph.type === "tmtrainer")
    ...
    displayedInfo() {
      const blacklist = ["companion", "cursed", "tmtrainer"];
  ...
  methods: {
    ...
    glyphEffectDots(id) {
      if (["companion", "cursed", "tmtrainer"].includes(this.glyph.type)) return {};
    ...
    glyphBorderStyle() {
      if (!this.showBorders || this.glyph.type === "tmtrainer") return null;
      let borderAttrs;
