...
export default {
  ...
  computed: {
    ...
    typeCapitalized() {
      if(this.type === "tmtrainer")
        return window.RANDOM_TMTRAINER_NAME(window.TMTRAINER_SHORTNAME_SEED).substring(0,8);
    ...
    levelText() {
      if (this.type === "tmtrainer")
        return window.RANDOM_TMTRAINER_LEVEL(window.TMTRAINER_SHORTLEVEL_SEED).substring(0,5);
    ...
    effectStyle() {
      return {
        "font-size": `${(this.type === "effarig" || this.type === "tmtrainer") ? 1 : 1.2}rem`,
    ...
    rarityPercent() {
      if (this.glyph.type === "companion" || this.glyph.type === "cursed" || this.glyph.type === "tmtrainer") return "";
